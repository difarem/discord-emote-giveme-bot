...
use discord::model::{Channel, ChannelId, EmojiId, MessageId, ReactionEmoji, RoleId, ServerId,
                     UserId};
...
fn main() {
    ...
    let roles: Vec<Role> = cfg.get_array("roles")
        .unwrap()
        .into_iter()
        .map(|r| r.into_table().unwrap())
        .map(|m| Role {
            role_id: RoleId(m["role"].clone().try_into().unwrap()),
            emoji: ReactionEmoji::Custom {
                name: m["name"].clone().into_str().unwrap(),
                id: EmojiId(m["id"].clone().try_into().unwrap()),
            },
        })
        .collect();
    let mut i: i32 = -1;
    ...
    let channels: Vec<(ChannelId, MessageId)> = cfg.get_array("messages")
        .unwrap()
        .into_iter()
        .map(|r| r.into_table().unwrap())
        .map(|m| {
            (
                ChannelId(m["channel"].clone().try_into().unwrap()),
                MessageId(m["id"].clone().try_into().unwrap()),
            )
        })
        .collect();
    for &(channel_id, message_id) in &channels {
        ...
        for reaction in message.reactions {
            ...
            loop {
                let reactions =
                    dc.get_reactions(channel_id, message_id, reaction.emoji.clone(), None, None)
                        .unwrap();
        ...
        for _ in 0..REACTION_CAP {
            if i >= roles.len() as i32 {
                ...
                dc.add_reaction(
                    channel_id,
                    message_id,
                    ReactionEmoji::Unicode(X.to_string()),
                ).unwrap();
            } else {
                dc.add_reaction(channel_id, message_id, roles[i as usize].emoji.clone())
                    .unwrap();
            }
